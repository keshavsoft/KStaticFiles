...
let CallBackFunc = (err, inFolderPath, inDestinationPath, results) => {
    ...
    results.forEach(element => {
        ...
        if (LoopInsidePath.ext === ".html") {
            ...
            //            console.log("vLoopInsidePath : ", LoopInsidePath, inFolderPath, element.replace(inFolderPath, inDestinationPath));
            console.log("vLoopInsidePath : ", element);
            fs.copySync(element, element.replace(inFolderPath, inDestinationPath));

            // fs.copy(inFolderPath, element.replace(inFolderPath, inDestinationPath), { overwrite: false, errorOnExist: true });

            // fs.copy(src, dest, { overwrite: false, errorOnExist: true });

            // if (fs.existsSync(element.replace(inFolderPath, inDestinationPath)) === false) {
            //     fs.createFileSync(element.replace(inFolderPath, inDestinationPath));
            // };

            // fs.copySync( inFolderPath,  element.replace(inFolderPath, inDestinationPath), (err) =>{
            //     if(err) throw err;
            // });
        };
    });
